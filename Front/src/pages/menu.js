...
const Menu = ()=>{
    ...
    return <div className="flex flex-col  text-white gap-8 ml-3 mt-[90%] ">
        <div ><Link className="text-3xl flex" to="/Groups">{group} Group </Link></div>
        <div ><Link className="text-3xl flex" to="/=">{project} Project </Link></div>
        <div ><Link className="text-3xl flex" to="/=">{timer} Timer </Link></div>
    </div>
}
export default Menu