...
export const addGroupToProject = async (projectId, admin, groups) => {
    const res = await axios.patch(`http://localhost:3000/projects/${projectId}`, {admin: admin, groups: groups} ,{new: true});
    return res.data;
}
...
export const deleteGrouptoProject = async (projectId, admin, groups) => {
    const res = await axios.patch(`http://localhost:3000/projects/${projectId}/deleteGroups`, {admin: admin, groups: groups},{new: true});
    return res.data;
}
